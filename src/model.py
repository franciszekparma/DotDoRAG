...
ettin = Ettin().to(device)
bge = BGE().to(device)

token_adder = AddTokens()

ettin_tokenizer = AutoTokenizer.from_pretrained("jhu-clsp/ettin-encoder-150m")
ettin_tokenizer.add_tokens(list(token_adder.new_tokens.values()))
ettin.enc.resize_token_embeddings(len(ettin_tokenizer))
