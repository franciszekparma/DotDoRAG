...
def main():
  ...
  encoded_path = sys.argv[2] if len(sys.argv) > 2 else utils.ENCODED_PATH
  threshold = float(sys.argv[3]) if len(sys.argv) > 3 else utils.DEFAULT_THRESHOLD
  ...
  while True:
    ...
    with torch.inference_mode():
      que = tokenizer(
        ...
        padding=True,
        truncation=True,
        max_length=utils.MAX_QUERY_LENGTH,
        return_tensors='pt',
      )
