...
from model import ettin, ettin_tokenizer, device
from utils import (
  ETTIN_LORA_MAX_LEN,
  ETTIN_LORA_BATCH_SIZE,
  ETTIN_LORA_RANK,
  ETTIN_LORA_ALPHA,
  ETTIN_LORA_DROPOUT,
  ETTIN_LORA_LR,
  ETTIN_LORA_EPOCHS,
  ETTIN_LORA_TEMP,
  ETTIN_LORA_WARMUP_RATIO,
)


class MultiNCELoss(nn.Module):
  def __init__(self, temp=ETTIN_LORA_TEMP):
    super().__init__()
    self.temp = temp
...
def collate_fn(batch):
  ...
  def tokenize(texts):
    return ettin_tokenizer(
      texts,
      padding=True,
      truncation=True,
      max_length=ETTIN_LORA_MAX_LEN,
      return_tensors='pt'
    )
...
def main():
  ...
  dl_kw = dict(batch_size=ETTIN_LORA_BATCH_SIZE, collate_fn=collate_fn)
  ...
  peft_config = LoraConfig(
    r=ETTIN_LORA_RANK,
    lora_alpha=ETTIN_LORA_ALPHA,
    target_modules="all-linear",
    lora_dropout=ETTIN_LORA_DROPOUT,
    bias="none",
    task_type=None,
  ...
  optimizer = torch.optim.AdamW(model.parameters(), lr=ETTIN_LORA_LR)
  loss_fn = MultiNCELoss()

  epochs = ETTIN_LORA_EPOCHS

  total_steps = len(train_dl) * epochs
  warmup_steps = int(ETTIN_LORA_WARMUP_RATIO * total_steps)
