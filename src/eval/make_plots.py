...
PRETTY = {
  'plain_ettin': 'plain Ettin',
  'ettin_lora':  'Ettin + LoRA',
  'plain_bge':   'plain BGE',
  'BM25':        'BM25',
  'bge_lora':    'BGE + LoRA',
}
...
def plots_from_eval_json():
  with open(os.path.join(BASE, 'eval_results.json')) as f:
    data = json.load(f)
  order = ['plain_ettin', 'ettin_lora', 'plain_bge', 'BM25', 'bge_lora']
...
def plots_from_model():
  ...
  enc, tok = _load_bge(adapter=os.path.join(BASE, 'adapters', 'bge_lora'))
  ...
  def pos_neg_sims(qv, pv, nv):
    pos_sim = (qv * pv).sum(1).numpy()
    neg_sim = (qv @ nv.T).numpy().reshape(-1)
