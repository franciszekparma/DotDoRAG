...
def load_ettin_with_adapter(adapter_dir, ettin_tok):
  enc = AutoModel.from_pretrained(ETTIN_NAME)
  enc.resize_token_embeddings(len(ettin_tok))
  enc = enc.to(device).eval()
  enc = PeftModel.from_pretrained(enc, adapter_dir).to(device).eval()
  return enc
...
def main():
  ...
  print('\n=== bge_lora ===')
  bge_enc = load_bge_with_adapter(os.path.join(repo_root, 'adapters', 'bge_lora'), bge_tok)
  ...
  ettin_tok = AutoTokenizer.from_pretrained(ETTIN_NAME)
  ettin_tok.add_tokens(list(AddTokens().new_tokens.values()))

  print('\n=== plain Ettin (no adapter) ===')
  plain_ettin = AutoModel.from_pretrained(ETTIN_NAME)
  plain_ettin.resize_token_embeddings(len(ettin_tok))
  plain_ettin = plain_ettin.to(device).eval()
  m = eval_dense(plain_ettin, ettin_tok, raw_queries, raw_corpus, corpus_ids,
                 graded, query_ids, q_prefix='', qmax=EVAL_ETTIN_MAX_LEN, dmax=EVAL_ETTIN_MAX_LEN, k=K)
  print(f'plain_ettin: ndcg@{K}={m[f"ndcg@{K}"]:.4f} recall@{K}={m[f"recall@{K}"]:.4f} roc_auc={m["roc_auc"]:.4f} auc_pr={m["auc_pr"]:.4f}')
  ...
  print('\n=== ettin_lora ===')
  ettin_enc = load_ettin_with_adapter(os.path.join(repo_root, 'adapters', 'ettin_lora'), ettin_tok)
  m = eval_dense(ettin_enc, ettin_tok, tagged_queries, tagged_corpus, corpus_ids,
                 graded, query_ids, q_prefix='', qmax=EVAL_ETTIN_MAX_LEN, dmax=EVAL_ETTIN_MAX_LEN, k=K)
  print(f'ettin_lora: ndcg@{K}={m[f"ndcg@{K}"]:.4f} recall@{K}={m[f"recall@{K}"]:.4f} roc_auc={m["roc_auc"]:.4f} auc_pr={m["auc_pr"]:.4f}')
  results.append(('ettin_lora', m))
  ...
  meta = {
    ...
    'bge_lora': ('bge_lora', 'dense', BGE_NAME, 'adapters/bge_lora/adapter_model.safetensors'),
    'plain Ettin (zero-shot)': ('plain_ettin', 'dense', ETTIN_NAME, None),
    'ettin_lora': ('ettin_lora', 'dense', ETTIN_NAME, 'adapters/ettin_lora/adapter_model.safetensors'),
  }
  payload = {
    'dataset': 'NFCorpus',
    'split': 'TEST',
